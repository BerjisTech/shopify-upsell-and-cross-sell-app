...
<script type="text/javascript">
    jQuery(document).ready(function() {
        ...
        jQuery(".sales").sparkline([0,
            <?php
            foreach ($this->db->select('sum(price) as stat, date_format(from_unixtime(date), "%d") as day, date_format(from_unixtime(date), "%Y %m %d") as year')->where('shop', $duka)->where('offer', $offer)->where('type', 'purchase')->group_by('day')->order_by('year', 'asc')->limit('30')->get('stats')->result_array() as $fetch) {
                echo $fetch['stat'] . ',';
            }
            ?>
        ], {
        ...
        jQuery(".customer-reach").sparkline([0,
            <?php
            foreach ($this->db->select('count(stat_id) as stat, date_format(from_unixtime(date), "%d") as day, date_format(from_unixtime(date), "%Y %m %d") as year')->where('shop', $duka)->where('offer', $offer)->where('type', 'impression')->group_by('day')->order_by('year', 'asc')->limit('30')->get('stats')->result_array() as $fetch) {
                echo $fetch['stat'] . ',';
            }
            ?>
        ], {
        ...
        jQuery(".all-time-sales").sparkline([0,
            <?php
            foreach ($this->db->select('count(stat_id) as stat, date_format(from_unixtime(date), "%d") as day, date_format(from_unixtime(date), "%Y %m %d") as year')->where('shop', $duka)->where('offer', $offer)->where('type', 'show')->group_by('day')->order_by('year', 'asc')->limit('30')->get('stats')->result_array() as $fetch) {
                echo $fetch['stat'] . ',';
            }
            ?>
        ], {
                ...
                data: [
                    ...
                    for ($m = 48; $m > 23; $m--) {
                        ...
                        $lastmonth = strtotime(date('Y-m-d H:m:s', strtotime('-' . ($m - 1) . ' hours'))); ?> {
                            ...
                            a: <?php
                                $where = "`shop` = '" . $duka . "' AND `offer` = '" . $offer . "' AND `type` = 'show' AND `date` BETWEEN '" . $nowmonth . "' AND '" . $lastmonth . "'";
                                ...
                                ?>,
                            b: <?php
                                $where = "`shop` = '" . $duka . "' AND `offer` = '" . $offer . "' AND `type` = 'impression' AND `date` BETWEEN '" . $nowmonth . "' AND '" . $lastmonth . "'";
                                ...
                                ?>,
                            c: <?php
                                $where = "`shop` = '" . $duka . "' AND `offer` = '" . $offer . "' AND `type` = 'purchase' AND `date` BETWEEN '" . $nowmonth . "' AND '" . $lastmonth . "'";
...
<div class="row">
    ...
    <div class="col-md-4 col-sm-6">
        <div class="tile-stats tile-white stat-tile">
            <h3>$ <?php echo number_format($this->db->select('sum(price) as total')->where('shop', $duka)->where('offer', $offer)->where('type', 'purchase')->get('stats')->row()->total,2,'.'); ?></h3>
